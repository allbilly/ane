...
C = 3           # Cin = Cout = 3 (3-channel 1x1 conv)
...
BTSP_BUF = make_from_segments(0x4000, [
    ...
    (0, 44, build_seg(0, 44, [
        ...
        (reg.W6,  # flags: next_priority=38
            ...
            (3 << 28)),     # pad bits
        (reg.W7, 0),  # next_ptr
        (reg.W8,  # base_ene: conv mode, rbase0=5, rbe0=1, wbase=36, el0_en=1
            (5) |          # rbase0=5
            (1 << 5) |     # rbe0=1
            (36 << 12) |   # wbase=36
            (1 << 24) |    # el0_en=1
...
output = np.frombuffer(out_map, dtype=np.float16, count=C * STRIDE).reshape(C, STRIDE)[:, 0].copy()
print("output =", output)
expected = np.full(C, (input_a[:C * STRIDE:STRIDE].sum() * 2.0).astype(np.float16))
