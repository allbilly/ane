...
commands = {
    reg.InDim: 0x10001, reg.pad0: 1, reg.ChCfg: 0x22, reg.Cin: 0x200, reg.Cout: 0x200,
    reg.OutDim: 0x10001, reg.pad1: 1, reg.ConvCfg: 0x5000b421, reg.pad2: 0x2041,
    reg.GroupConvCfg: 0x10001, reg.TileCfg: 1, reg.pad3: 0, reg.pad4: 0, reg.Cfg: 0x244405,
    ...
    reg.ResultCfg: 0x500172, reg.ResultBase: 0x2030,
    ...
    reg.SrcRowStride: 0x40, reg.SrcPlaneStride: 0x40, reg.SrcDepthStride: 0x8000,
    ...
    reg.DstPlaneStride: 0x40, reg.DstDepthStride: 0x8000, reg.DstGroupStride: 0,
    reg.DstFmt: 0x1302031,
}

with open('hwx/tinygrad/gemm.hwx', 'rb') as f:
    raw = f.read()

# Parse Mach-O to find ane_data and __TEXT.__const (weights)
import sys; sys.path.insert(0, os.path.join(os.path.dirname(__file__), '..'))
from hwx_parsing import parse_macho
ane_data = parse_macho(raw)

# Find __TEXT.__const section offset and inject non-zero weights
ncmds = struct.unpack_from('<I', raw, 16)[0]
off = 32
const_off = const_size = 0
for i in range(ncmds):
    if off + 8 > len(raw): break
    cmd = struct.unpack_from('<I', raw, off)[0]
    if cmd == 0x19:
        segname = raw[off+8:off+24].strip(b'\x00').decode(errors='ignore')
        nsects = struct.unpack_from('<I', raw, off+64)[0]
        sect_off = off + 72
        for j in range(nsects):
            sname = raw[sect_off:sect_off+16].strip(b'\x00').decode(errors='ignore')
            ssize = struct.unpack_from('<Q', raw, sect_off+40)[0]
            soff = struct.unpack_from('<I', raw, sect_off+48)[0]
            if segname == '__TEXT' and sname == '__const':
                const_off, const_size = soff, ssize
            sect_off += 80
    off += struct.unpack_from('<I', raw, off+4)[0]

# Inject non-zero weights (0.5 for all 512×512 entries)
k = np.full(const_size // 2, np.float16(0.5), dtype=np.float16)
kernel = k.tobytes()
...
print(f"NOTE: GEMM output = sum(512 inputs × 0.5) × input_val. With all inputs=1.0, expected ≈ {0.5*512*1.0:.0f}")
os.close(fd)
