...
# Task descriptor: tid=0, nid=64, eon=1. KernelDMA = stream_header(0x1F800, 62) loads
# 62 FW DMA words from 0x2C. Those 62 words are all zeros (no firmware modules loaded).
#
# Key difference vs relu.py: SrcStream at host 0x168 points to ANE 0x04800 (L2 controller)
# instead of 0x13800 (TileDMA Src). L2 cache handles input data sourcing instead of TileDMA.
...
# Overwrite specific register offsets with correct values
commands = {
    ...
    reg.SourceChannelStride: 0, reg.SourceRowStride: ST,
    reg.L2pad0: ST, reg.L2pad1: ST, reg.L2pad2: 0,
    ...
    reg.DstDMAConfig: 0x040000c1, reg.DstBaseAddr: 0, reg.DstRowStride: ST,
    reg.DstPlaneStride: ST, reg.DstDepthStride: ST, reg.DstGroupStride: 0,
...
ret = submit_task(fd, 0x274, 1, 0x274, [btsp_handle, 0, 0, 0, out_handle, src1_handle, 0] + [0] * 25, btsp_handle)
os.close(fd)
